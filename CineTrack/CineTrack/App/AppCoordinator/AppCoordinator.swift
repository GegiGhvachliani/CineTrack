...
final class AppCoordinator: AppCoordinatorProtocol {
    ...
    private func showOnboardingFlow() {
        let onboardingCoordinator = container.onboardingFactory.makeOnboardingCoordinator(
    ...
    private func showAuthFlow() {
        let authCoordinator = container.authenticationFactory.makeAuthenticationCoordinator(
            navigationController: rootNavigationController)
    ...
    private func showMainFlow() {
        let tabBarCoordinator: MainTabBarCoordinatorProtocol = MainTabBarCoordinator(
            navigationController: rootNavigationController,
            container: container
        )
        tabBarCoordinator.onSignedOut = { [weak self] in
            guard let self else { return }
            self.childCoordinators.removeAll { $0 is MainTabBarCoordinatorProtocol }
