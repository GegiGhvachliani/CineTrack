    ...
    public var body: some View {
        Button(action: action) {
            Image(systemName: "bookmark.fill")
                .resizable()
                .scaledToFit()
                .foregroundStyle(isAdded ? ColorTokens.Brand.primary : ColorTokens.Media.scrim.opacity(0.5))
                .frame(width: size)
                .padding(1)
                ...
                .overlay {
                    Image(systemName: isAdded ? "checkmark" : "plus")
                        .resizable()
                        .scaledToFit()
                        .foregroundStyle(isAdded ? ColorTokens.Text.onBrand : ColorTokens.Text.onImage)
