...
extension ActorDetailsViewModel {
    ...
    private func load(force: Bool) async {
        ...
        async let historyTask: Void = recordRecentlyViewed(actor)
    ...
    private func recordRecentlyViewed(_ actor: ActorDetails) async {
        do {
            try await addRecentlyViewedActorUseCase.execute(
                RecentlyViewedActor(
                    id: actor.id,
                    name: actor.name,
            ...
            sectionErrors[.recentlyViewed] = error
        }
    }
