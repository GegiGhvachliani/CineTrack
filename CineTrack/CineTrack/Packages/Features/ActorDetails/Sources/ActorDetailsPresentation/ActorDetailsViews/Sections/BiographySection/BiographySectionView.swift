...
public struct BiographySectionView: View {
    ...
    private var addToFavouritesButton: some View {
        Button {
            ...
            HStack(spacing: 8) {
                ...
                Text(
                    isFavourite
                        ? ActorDetailsStrings.Content.addedToFavorites : ActorDetailsStrings.Content.addToFavorites
                )
    ...
    private var bornText: String? {
        ...
        return ActorDetailsStrings.Format.born(date: formattedDate(birthday))
    }
    ...
    private var diedText: String? {
        ...
        if let birthday = actor.birthday {
            ...
            ageText = ActorDetailsStrings.Format.aged(age: age)
        } else {
            ageText = ""
        }

        return ActorDetailsStrings.Format.died(date: formattedDate(deathday), age: ageText)
    }
